...
################ CDF depth quantile table

fmt1 <- function(x, dec.points = 0){
  ## Format x to have supplied number of decimal points
  ## Make thousands seperated by commas and the number of decimal points given by
  ##  dec.points
  return(format(round(x,dec.points), big.mark = ",", nsmall = dec.points))
...
make.cdfquantdepth.table <- function(spec,
                                     digits = 2,           ## Number of decimal places
                                     xcaption = "default", ## Caption to use
                                     xlabel   = "default", ## Latex label to use
                                     font.size = 9,        ## Size of the font for the table
  ...
  row.names <- paste0("\\textbf{", gsub("\\.", " ", row.names), "}")
  spec <- cbind(row.names, fmt1(spec, digits))
  ...
  addtorow$pos[[1]] <- -1
  addtorow$pos[[2]] <- 0
  addtorow$command <- c(paste0("& \\multicolumn{", ncol(spec) - 1, "}{c}{\\textbf{Quantile}} \\\\ "),
                        paste0(" & \\textbf{2.5\\%}",
                               " & \\textbf{25\\%}",
                               " & \\textbf{50\\%}",
                               " & \\textbf{75\\%}",
  ...
  ## Make the size string for font and space size
  size.string <- paste0("\\fontsize{",font.size,"}{",space.size,"}\\selectfont")
  return(print(xtable(spec,
                      caption = xcaption,
                      label = xlabel,
                      align = get.align(ncol(spec))),
               caption.placement = "top",
               include.rownames = FALSE,
               include.colnames = FALSE,
               add.to.row = addtorow,
...
spec <- read.csv("C:/GitHub/SPERA-Maps/Results/Figures/Quantiles_Depth.csv")
spec <- na.omit(spec)
make.cdfquantdepth.table(spec,## The output of the load catches function above.
                         digits = 2, ## number of digits to show
                         xcaption = "default", ## Caption to use
                         xlabel   = "default", ## Latex label to use
                         font.size = 9,        ## Size of the font for the table
